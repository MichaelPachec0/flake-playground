# home-manager module: cspell config (~/.config/cspell/cspell.json).
# Editor-agnostic, but pairs with the cspell-nvim plugin. Dictionary packs come
# from the cspell-dicts input (flake = false). Migrated out of nix-config's old
# nixneovim setup.
inputs: {
  config,
  lib,
...
in {
  options.programs.cspell = {
    enable = mkEnableOption ''cspell config written to ~/.config/cspell/cspell.json'';
  };
